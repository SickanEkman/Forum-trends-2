...
class Tfidf:
    def __init__(self, prepared_forum):
        ...
        self.sorted_tfidf = self.count_tfidf(self.idf_posts_as_doc)
    ...
    def create_new_doc_from_sorted(self):
        half_the_list = len(self.sorted_tfidf) // 2
        print(len(self.sorted_tfidf))
        for word in self.sorted_tfidf[:half_the_list]:
            print(word[0])
